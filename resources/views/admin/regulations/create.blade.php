...
<section class="content clearfix">
    <form method="post" action="{{ route('admin.regulations.store') }}" method="POST" enctype="multipart/form-data">
     @csrf
 <h4 class="pull-left">الشروط والاحكام
  <small>انشاء الشروط والاحكام</small>
</h4>

<div class="pull-right">
  <div class="col-md-12 text-right">

    <button type="submit" class="btn btn-primary margin-r-5" ><i class="fa fa-save margin-r-5"></i>انشاء الشروط والاحكام</button>
    ...
    $(document).ready(function() {
        $('.deleteImage').click(function() {
            var imageID = $(this).val();
                $.ajax({
                    url: '/admin/regulations/image/ajax/'+imageID,
                    type: "GET",
                    dataType: "json",
    ...
    $(document).ready(function() {
        $('.deleteTitleSound').click(function() {
            var soundID = $(this).val();
                $.ajax({
                    url: '/admin/regulations/sound/ajax/'+soundID,
                    type: "GET",
                    dataType: "json",
                    success:function(data) {
                        console.log('deleted');
    ...
    $(document).ready(function() {
        $('.deleteTitleVideo').click(function() {
            var videoID = $(this).val();
                $.ajax({
                    url: '/admin/regulations/video/ajax/'+videoID,
                    type: "GET",
                    dataType: "json",
                    success:function(data) {
                        console.log('deleted');
